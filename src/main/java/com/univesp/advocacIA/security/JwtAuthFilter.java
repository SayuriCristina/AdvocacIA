...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private boolean isPublicEndpoint(String path, String method) {
        ...
        if (Arrays.stream(PUBLIC_ENDPOINTS).anyMatch(pattern -> matcher.match(pattern, path))) {
            return true;
        }
        if ("GET".equals(method) && Arrays.stream(PUBLIC_GET_ENDPOINTS).anyMatch(pattern -> matcher.match(pattern, path))) {
            return true;
        }
        return false;
    }
